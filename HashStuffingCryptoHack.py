...
ZERO32 = b'\x00' * BLOCK_SIZE

def scramble_block_linear(block):
    for _ in range(40):
        block = xor(ZERO32, block)  
        block = rotate_left(block, 6)
        block = xor(ZERO32, block)  
        block = rotate_right(block, 17)
    return block
...
def L_i(delta, i):
    def F_i(block):
        ...
        for _ in range(i):
            ...
            mix_in = rotate_left(mix_in, i+6)
        return mix_in
    return xor(F_i(delta), F_i(bytes(BLOCK_SIZE)))

def find_shift(i):
    probe = bytes([0xFF] + [0]*31)
    return L_i(probe, i).index(0xFF)

sh0 = find_shift(0)
sh1 = find_shift(1)

block0 = os.urandom(32)
block1 = os.urandom(32)
msg = block0 + block1              

delta0 = os.urandom(32)            
delta1 = rotate_left(rotate_right(delta0, sh0), sh1)  
